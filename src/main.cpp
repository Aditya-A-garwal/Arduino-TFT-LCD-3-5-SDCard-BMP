...
const int PALETTEDEPTH = 0;
const int BMPIMAGEOFFSET = 54;
const int BUFFPIXEL = 20;

// uint8_t showBMP(char *, int, int);

char *strlwr(char *src) __attribute__((weak));
...
void setup() {
    ...
    char fullpath[64] = "/";
    char *filename = &fullpath[1];

    uint8_t err;

    root    = SD.open("/"); // open the root directory in the SD Card
    ...
    for (;;) {
        ...
        if (current == NULL) {
            ...
            Serial.println("NO BMP FILES TO SHOW");
            for (;;);
        }

        strcpy(filename, (char *)current.name());
        current.close();
